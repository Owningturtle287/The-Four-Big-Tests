import {DEFINITIONS,LEGACY_DARK_VERSION,assessmentItems} from './scoring.js';
import {makeQuestion,FAMILIES,IQ_VERSION,fingerprint} from './iq.js';
import {makeQuestion as makeLegacyQuestion,FAMILIES as LEGACY_FAMILIES,IQ_VERSION as LEGACY_IQ_VERSION} from './iq-v1.js';
...
export function validateBackup(data,idFactory=()=>crypto.randomUUID()) {
  ...
  return data.attempts.map(raw=>{
    ...
    if(raw.version!==DEFINITIONS[raw.type].version&&!(raw.type==='iq'&&raw.version===LEGACY_IQ_VERSION)&&!(raw.type==='dark'&&raw.version===LEGACY_DARK_VERSION))throw new Error('A session uses an unsupported question version. Keep the original backup; this version cannot safely rescore it.');
    const iq=raw.type==='iq',legacy=iq&&raw.version===LEGACY_IQ_VERSION,sectionSize=legacy?8:10,families=legacy?LEGACY_FAMILIES:FAMILIES;let items;
    if(iq){
      if(!Array.isArray(raw.items)||raw.items.length!==(legacy?40:50))throw new Error('A reasoning session has an invalid question count.');
      const familyCounts={};
      items=raw.items.map((q,i)=>{
        if(!plain(q)||q.domain!==Object.keys(families)[Math.floor(i/sectionSize)]||!families[q.domain].includes(q.family)||!Number.isInteger(q.seed)||q.seed<0||q.seed>0xFFFFFFFF||!(legacy?[1,2]:families[q.domain].indexOf(q.family)<4?[1,2]:[3]).includes(q.level))throw new Error('Invalid reasoning question parameters.');
        const k=`${q.domain}/${q.family}/${q.level}`;familyCounts[k]=(familyCounts[k]||0)+1;
        return legacy?makeLegacyQuestion(q.domain,q.family,q.seed,q.level):makeQuestion(q.domain,q.family,q.seed,q.level);
      });
      if(Object.keys(familyCounts).length!==items.length||Object.values(familyCounts).some(n=>n!==1)||new Set(items.map(fingerprint)).size!==items.length)throw new Error('Unbalanced reasoning form.');
    } else items=assessmentItems(raw.type,raw.version);
    ...
    for(const [k,v] of Object.entries(raw.answers)){
      if(!ids.has(k)||!(iq?Number.isInteger(v)&&v>=-1&&v<=(legacy?3:5):raw.type==='politics'&&v===null||Number.isInteger(v)&&v>=1&&v<=5))throw new Error('Invalid saved response.');
      answers[k]=v;
    }
    ...
    if(iq&&raw.status==='draft'&&Math.floor(raw.index/sectionSize)!==raw.section)throw new Error('Question and section do not match.');
